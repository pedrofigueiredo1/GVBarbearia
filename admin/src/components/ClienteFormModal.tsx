...
interface Props {
  ...
  onClose: () => void;
  onSalvar: (data: ClienteInput) => Promise<void>;
}
...
const campoVazio: FormState = { nome: '', email: '', telefone: '', senha: '' };

// Um único modal serve tanto para o cadastro (feito pelo admin, quando o
// cliente prefere ser atendido presencialmente em vez de usar o app) quanto
// para a edição. Na edição, a senha fica opcional: em branco significa
// "manter a atual".
export function ClienteFormModal({ cliente, onClose, onSalvar }: Props) {
  const [form, setForm] = useState<FormState>(
    cliente
      ? { nome: cliente.nome, email: cliente.email, telefone: cliente.telefone, senha: '' }
      : campoVazio,
  );
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1" htmlFor="senha">
              {cliente ? 'Nova senha (deixe em branco para manter a atual)' : 'Senha'}
            </label>
            <input
              id="senha"
              type="password"
